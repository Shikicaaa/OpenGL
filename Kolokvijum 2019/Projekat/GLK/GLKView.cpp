...
void CGLKView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	switch (nChar) {
	case VK_RIGHT:
		m_glRenderer.m_camBeta -= 1;
		break;
	case VK_LEFT:
		m_glRenderer.m_camBeta += 1;
		break;
	case VK_UP:
		if (m_glRenderer.m_camAlpha < 89)
			m_glRenderer.m_camAlpha += 1;
		break;
	case VK_DOWN:
		if (m_glRenderer.m_camAlpha > -89)
			m_glRenderer.m_camAlpha -= 1;
	...
	case VK_OEM_PLUS:
	case VK_ADD:
		if (m_glRenderer.m_camDist > 0)
			m_glRenderer.m_camDist -= 1;
		break;
	case VK_OEM_MINUS:
	case VK_SUBTRACT:
		if (m_glRenderer.m_camDist < 50)
			m_glRenderer.m_camDist += 1;
		break;
	case '1':
		m_glRenderer.m_rotKabina += 3;
		break;
	case '2':
		m_glRenderer.m_rotKabina -= 3;
		break;
	case '3':
		m_glRenderer.m_rotRuka1 += 3;
		break;
	case '4':
		m_glRenderer.m_rotRuka1 -= 3;
		break;
	case '5':
		m_glRenderer.m_rotRuka2 += 3;
		break;
	case '6':
		m_glRenderer.m_rotRuka2 -= 3;
		break;
	case '7':
		m_glRenderer.m_rotViljuska += 3;
		break;
	case '8':
		m_glRenderer.m_rotViljuska -= 3;
		break;
