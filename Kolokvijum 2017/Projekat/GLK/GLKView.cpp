...
void CGLKView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	switch (nChar) {
	case VK_LEFT:
		m_glRenderer.m_camBeta += 10.0;
		break;
	case VK_RIGHT:
		m_glRenderer.m_camBeta -= 10.0;
		break;
	case VK_UP:
		if (m_glRenderer.m_camAlpha > 0)
			m_glRenderer.m_camAlpha -= 10.0;
		break;
	case VK_DOWN:
		if (m_glRenderer.m_camAlpha < 89)
			m_glRenderer.m_camAlpha += 10.;
		break;
	case VK_OEM_PLUS:
	case VK_ADD:
		m_glRenderer.m_camDist -= 10.0;
		break;
	case VK_OEM_MINUS:
	case VK_SUBTRACT:
		m_glRenderer.m_camDist += 10.0;
	...
	case 'q':
	case 'Q':
		m_glRenderer.m_rotZupcanik += 10.0;
		break;
	case 'w':
	case 'W':
		m_glRenderer.m_rotZupcanik -= 10.0;
